  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 py-20">
      ...
      <div className="w-full md:w-1/2 flex justify-center">
        <div className="relative w-64 h-64 md:w-80 md:h-80 rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.1)] border border-slate-500/30 backdrop-blur-sm avoid-zone">
      ...
      <div className="w-full md:w-1/2 flex flex-col gap-6 text-center md:text-left avoid-zone">
        ...
        <h3 className="text-3xl md:text-5xl font-bold opacity-90 leading-tight">
          {aboutData.mainTitle}
        </h3>
        ...
        <div className="flex flex-wrap gap-4 mt-4 justify-center md:justify-start">
          {aboutData.highlights.map((highlight, index) => (
            <div key={index} className="px-5 py-2 rounded-full border border-slate-500/30 bg-slate-500/10 backdrop-blur text-sm opacity-80">
              {highlight}
            </div>
          ))}
        </div>
